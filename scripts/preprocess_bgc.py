...
UPDATED: Now outputs Samudra-compatible dimension names (y, x, lev instead of yh, xh, z_l)
...
- RENAME DIMENSIONS: yh->y, xh->x, z_l->lev (Samudra compatibility)
- Create impermeable walls via a boundary ring (mask_*, wetmask) ONLY FOR FULL DOUBLE GYRE Model
...
For ds_mini.zarr :  !!! --bw = 1 because boundaries are impermeable 

Run (on Tiger HPC) : 
module load anaconda3/2024.10
conda activate samudra-bgc
python preprocess_bgc.py   --input /scratch/gpfs/GEOCLIM/LRGROUP/maximek/INMOS/prototypes_dataset/ds_mini.zarr   --output /scratch/gpfs/GEOCLIM/LRGROUP/maximek/INMOS/prototypes_dataset/processed_data   --years 9   --bw 1 
...
def rename_dimensions_for_samudra(ds: xr.Dataset) -> xr.Dataset:
    """
    Rename dimensions to match Samudra expectations:
    yh -> y, xh -> x, z_l -> lev
    """
    logging.info("Renaming dimensions for Samudra compatibility...")
    dim_rename_map = {}
    ...
    if "xh" in ds.dims:
        dim_rename_map["xh"] = "x"
    if "z_l" in ds.dims:
        dim_rename_map["z_l"] = "lev"
    
    if dim_rename_map:
        logging.info(f"Renaming dimensions: {dim_rename_map}")
        ds = ds.rename(dim_rename_map)
...
def make_closed_basin_masks(ds: xr.Dataset, boundary_width: int = 1) -> xr.Dataset:
    ...
    Produces mask_0..mask_{Nz-1} on (y,x) and a combined 3D wetmask.
    UPDATED: Uses (y,x) dimensions instead of (yh,xh)
    """
    # Check for both old and new dimension names
    y_dim = "y" if "y" in ds.dims else "yh"
    x_dim = "x" if "x" in ds.dims else "xh"
    z_dim = "lev" if "lev" in ds.dims else "z_l"
    
    assert y_dim in ds.dims and x_dim in ds.dims, f"Expected tracer grid ({y_dim},{x_dim}) in dims"
    assert z_dim in ds.dims, f"Expected vertical dim '{z_dim}'"

    Ny, Nx, Nz = ds.sizes[y_dim], ds.sizes[x_dim], ds.sizes[z_dim]
    ...
    for k in range(Nz):
        ds[f"mask_{k}"] = xr.DataArray(base.copy(), dims=(y_dim, x_dim))

    wet3d = np.stack([base] * Nz, axis=0)  # [z_dim,y_dim,x_dim]
    ds["wetmask"] = xr.DataArray(
        wet3d, dims=(z_dim, y_dim, x_dim),
        coords={z_dim: ds[z_dim]}
    )
    return ds


def split_3d(ds: xr.Dataset, var: str, zdim: str = None) -> xr.Dataset:
    """
    Split 3D var(time,z,y,x) into per-level channels var_0..var_{Nz-1}.
    UPDATED: Auto-detect vertical dimension name
    """
    if var not in ds:
        return ds
    ...
    if zdim is None:
        zdim = "lev" if "lev" in ds.dims else "z_l"
...
def write_zarr_v2(ds: xr.Dataset, store: str, clevel: int = 1):
    ...
    ds32.to_zarr(store, mode="w", consolidated=False, zarr_version=2, encoding=enc)
...
def prepare_idealized_bgc_data(
    ...
    # CRITICAL: Rename dimensions for Samudra compatibility BEFORE mask creation
    ds = rename_dimensions_for_samudra(ds)

    # Optional convenience: surface layer of thkcello
    if "thkcello" in ds and "lev" in ds.dims:
        ds["thkcello_surface"] = ds["thkcello"].isel(lev=0)

    # Closed-basin masks (impermeable walls) - now uses (y,x) dimensions
    ...
    logging.info(f"Final dataset dimensions: {ds_out.dims}")
    logging.info(f"Variables included: {list(ds_out.data_vars.keys())[:10]}...")
    return data_store
