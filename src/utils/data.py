...
from constants_bgc import DEPTH_I_LEVELS, DEPTH_LEVELS, MASK_VARS, TensorMap
...
class Normalize:
    ...
    def normalize_prognostics(
        ...
        norm = (data - self.prognostic_mean) / (self.prognostic_std + eps)
        if fill_nan:
    ...
    def normalize_boundary(
        ...
        eps = 1e-8  # Small epsilon to prevent division by zero
        norm = (data - self.boundary_mean) / (self.boundary_std + eps)
        if fill_nan:
            norm = norm.fillna(fill_value)
        return norm
    ...
    def normalize_tensor_prognostics(
        ...
        norm = (data - tensor_mean) / (tensor_std + eps)
        if fill_nan:
    ...
    def normalize_numpy_prognostics(
        ...
        # Use relative epsilon: max(1e-12, 1e-8 * abs(std)) to preserve small but non-zero variations
        eps = np.maximum(1e-12, 1e-8 * np.abs(self._prognostic_std_np))
        
        if data.ndim == 3:
            norm = (data - self._prognostic_mean_np) / (self._prognostic_std_np + eps)
        elif data.ndim == 4:
            eps_reshaped = eps.reshape(1, -1, 1, 1)
            norm = (
                data - self._prognostic_mean_np.reshape(1, -1, 1, 1)
            ) / (self._prognostic_std_np.reshape(1, -1, 1, 1) + eps_reshaped)
        if fill_nan:
